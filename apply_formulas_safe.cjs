...
async function processExcel() {
  ...
  if (summarySheet) {
    for (let i = 2; i <= lastRow - 1; i++) {
      ...
      const growthFormulaName = `IF(AND(${MAX_S}=5, ${MIN_S}=5), "Standard Achiever (ผู้บรรลุมาตรฐาน)", IF(AND(${MAX_S}=4, ${MIN_S}=4), "The Maintainer (ผู้ประคองงาน)", IF(AND(${MAX_S}<=3, ${MIN_S}=${MAX_S}), "Needs Attention (ผู้ต้องได้รับการดูแล)", IF(${MIN_S}>=4, "Generalist (ผู้เรียนรู้รอบด้าน)", IF(${MAX_S}<=3, "Apprentice (ผู้ฝึกหัด)", IF(AND(${C4}>0, ${C3}>0), "Rookie (ดาวรุ่ง)", "Uncalibrated (ศักยภาพที่รอการเจียระไน)"))))))`;
      
      const growthFormulaDesc = `IF(AND(${MAX_S}=5, ${MIN_S}=5), "ปฏิบัติงานได้ตามมาตรฐานอย่างครบถ้วนในทุกมิติ เป็นฟันเฟืองที่พึ่งพาได้ ควรกล้ารับความท้าทายใหม่ๆ เพื่อยกระดับสู่ผู้เชี่ยวชาญ", IF(AND(${MAX_S}=4, ${MIN_S}=4), "ประคองการทำงานภาพรวมได้ แต่มีช่องโหว่ในรายละเอียด ควรเน้นความรอบคอบและเรียนรู้จากพี่เลี้ยงเพื่อยกระดับผลงาน", IF(AND(${MAX_S}<=3, ${MIN_S}=${MAX_S}), "ผลการปฏิบัติงานต่ำกว่าเกณฑ์ในทุกมิติ หัวหน้างานควรเร่งประเมินสาเหตุและวางแผนปรับพื้นฐานใหม่ (Re-train) อย่างเร่งด่วน", IF(${MIN_S}>=4, "มีพื้นฐานที่สม่ำเสมอและปรับตัวได้ทุกบทบาท ควรผลักดันให้หา ""ความถนัดเฉพาะทาง"" 1-2 ด้าน เพื่อทะลุกำแพงสู่ระดับที่สูงขึ้น", IF(${MAX_S}<=3, "อยู่ในช่วงเริ่มต้นหรือยังไม่คุ้นเคยกับกระบวนการ ต้องการการสอนงานอย่างใกล้ชิด (OJT) และกำหนดเป้าหมายที่ชัดเจน", IF(AND(${C4}>0, ${C3}>0), "เริ่มฉายแววในด้านที่ถนัด แต่ทักษะอื่นยังไม่เสถียร ควรส่งเสริมจุดแข็งและใช้พี่เลี้ยงประคองจุดอ่อน", "ศักยภาพแฝงมีแต่ผลงานยังขาดความสม่ำเสมอ หัวหน้าควรช่วยจัดลำดับความสำคัญและแก้จุดอ่อนทีละจุดเพื่อให้ผลงานนิ่งขึ้น"))))))`;

      const growthFormulaIdentity = `IF(AND(${MAX_S}=5, ${MIN_S}=5), "The Standard", IF(AND(${MAX_S}=4, ${MIN_S}=4), "The Maintainer", IF(AND(${MAX_S}<=3, ${MIN_S}=${MAX_S}), "Needs Attention", IF(${MIN_S}>=4, "Undeveloped Potential", IF(${MAX_S}<=3, "The Beginner", IF(AND(${C4}>0, ${C3}>0), "Emerging Talent", "Uncalibrated"))))))`;
