...
export default function App() {
  ...
  // 🛠️ ระบบมาตรฐานชื่อโครงการ (ใช้ร่วมกันทั้งแอป)
  ...
  // 🛠️ เช็ค Overdue อิงจาก "วันที่" และ "เวลาตัดเกณฑ์"
  ...
  // เช็คสถานะเกินกำหนด สำหรับ UI (หน้าจอจะโชว์ว่าเกินกำหนด แต่ตอน Save จะไม่ Save คำนี้ทับลงไป)
  const isTaskOvd = (t, checkDate = getTStr()) => t.status !== 'จบงาน' && t.status !== 'ยกเลิก' && (t.overdueStatus === 'เกินกำหนด' || chkOvdTimeAware(t, checkDate));
  ...
  const subT = (e) => {
    ...
    // คงสถานะล่าช้าเดิมไว้ ไม่ล้างทิ้ง และไม่เซฟทับ เพื่อรอให้บอทจัดการ
  ...
  const cfSt = () => {
    ...
    if (t) {
        ...
        if(sMod.type === 'cancel') { nT.status = 'ยกเลิก'; nT.cancelReason = sMod.reason; nT.emailAlert = { action: 'ยกเลิกงาน', reason: sMod.reason, emails: getTargetEms(t.project), project: t.project, details: t.details }; }
            ...
            // แจ้งเตือนผู้ดูแลระบบทันที ถ้าปิดงานล่าช้ากว่ากำหนดของวันนี้
            if (chkOvdTimeAware(nT, getTStr()) || nT.completedDate > nT.endDate) { nT.overdueStatus = 'เกินกำหนด'; nT.emailAlert = { action: 'ปิดงานล่าช้ากว่ากำหนด', reason: `ปิดงานเวลา ${new Date().toLocaleTimeString('th-TH')} น. (เลยเวลาตัดเกณฑ์)`, emails: getTargetEms(nT.project), project: nT.project, details: t.details }; }
        }
        saveD('task', nT);
    }
  ...
  const addEmailMapping = () => {
    ...
    if (idx > -1) {
      ...
      if (pj === 'ทั้งหมด') { projs = ['ทั้งหมด']; } 
        ...
        if (!projs.includes(pj)) { if (projs.length >= 15) return alert('1 อีเมลสามารถผูกโครงการได้สูงสุด 15 โครงการครับ'); projs.push(pj); }
      }
  ...
  // 🛠️ แดชบอร์ดใหม่ แยกสถิติรายวัน/รายเดือน เป๊ะๆ 100%
  const rDash = () => {
    ...
    // สถิติเกินกำหนดแยกรายวัน / รายเดือน
    const mtOv = mt.filter(t => isTaskOvd(t, tS)); // รายเดือนเช็คกับวันนี้
    const dyOv = dy.filter(t => isTaskOvd(t, tD)); // รายวันเช็คกับวันที่เลือก
    ...
    return (
      <div className="space-y-6 animate-in">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white p-4 rounded-xl shadow-sm border-l-[4px] border-[#0f2e4a]">
            <div className="text-xs text-gray-500 font-bold mb-1">งานเดือน {tM}</div>
            <div className="text-2xl font-black">{mt.length}</div>
          </div>
          <div className="bg-white p-4 rounded-xl shadow-sm border-l-[4px] border-[#bca374]">
            <div className="text-xs text-gray-500 font-bold mb-1">งานวันที่ {fDate(tD)}</div>
            <div className="text-2xl font-black">{dy.length}</div>
          </div>
          <div onClick={()=>{ setGilt(p=>({...p, date: tS})); setOPop({isOpen:true, type:'daily'}); }} className="bg-red-50 p-4 rounded-xl shadow-sm border-l-[4px] border-red-500 cursor-pointer hover:shadow-md transition">
            <div className="text-xs text-red-600 font-bold mb-1 flex justify-between">ค้าง/ช้า (รายวัน) <span className="text-[9px] bg-red-200 px-1 rounded text-red-800">คลิกดู</span></div>
            <div className="text-2xl font-black text-red-700">{dyOv.length}</div>
          </div>
          <div onClick={()=>setOPop({isOpen:true, type:'monthly'})} className="bg-red-50 p-4 rounded-xl shadow-sm border-l-[4px] border-red-700 cursor-pointer hover:shadow-md transition">
            <div className="text-xs text-red-700 font-bold mb-1 flex justify-between">ค้าง/ช้า (รายเดือน) <span className="text-[9px] bg-red-200 px-1 rounded text-red-800">คลิกดู</span></div>
            <div className="text-2xl font-black text-red-800">{mtOv.length}</div>
          </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-white p-6 rounded-xl shadow-sm border">
            <SimplePieChart data={[{name:'จบงาน',value:dyDone,color:THEME.success},{name:'ดำเนินการ',value:Math.max(0, dyProg),color:THEME.secondary},{name:'เกินกำหนด',value:dyOv.length,color:THEME.danger}]} title={`สถานะงานรายวัน (${fDate(tD)})`}/>
          </div>
          <div className="bg-white p-6 rounded-xl shadow-sm border">
            <SimplePieChart data={[{name:'จบงาน',value:mtDone,color:THEME.success},{name:'ดำเนินการ',value:Math.max(0, mtProg),color:THEME.secondary},{name:'เกินกำหนด',value:mtOv.length,color:THEME.danger}]} title={`สถานะงานเดือนนี้ (${tM})`}/>
          </div>
        </div>
      </div>
    );
