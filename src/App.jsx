...
export default function App() {
  ...
  const cfSt = () => {
    ...
    if (t) {
        ...
        if(sMod.type === 'cancel') { nT.status = 'ยกเลิก'; nT.cancelReason = sMod.reason; nT.emailAlert = { action: 'ยกเลิกงาน', reason: sMod.reason, emails: getTargetEms(t.project), project: t.project, details: t.details }; }
            ...
            if (nT.completedDate > nT.endDate) nT.overdueStatus = 'เกินกำหนด';
        }
        saveD('task', nT);
    }
  ...
  const upS = (k, v, arr=true) => { let nS = {...sets}; if(arr) { const val = (v || '').trim(); if(!val || nS[k].includes(val)) return; nS[k] = [...nS[k], val]; setSInp(p => ({...p, [k]:'', projArea:'', slaDays:''})); } else { nS[k] = v; } saveD('settings', nS); };
  const dlS = (k, v) => { let nS = {...sets, [k]: sets[k].filter(x => x !== v)}; saveD('settings', nS); };
  const clearSList = (k) => { if(window.confirm('⚠️ ยืนยันการลบข้อมูล "ทั้งหมด" ในหมวดหมู่นี้ใช่หรือไม่?')) { let nS = {...sets, [k]: []}; saveD('settings', nS); } };
  
  const addEmailMapping = () => {
    const em = sInp.emails.trim(), pj = sInp.emProj || 'ทั้งหมด';
    if (!em) return;
    let nEms = [...sets.emails];
    const idx = nEms.findIndex(x => x.startsWith(em + '|'));
    
    if (idx > -1) {
      const parts = nEms[idx].split('|');
      let projs = parts[1] ? parts[1].split(',') : [];
      if (pj === 'ทั้งหมด') { projs = ['ทั้งหมด']; } 
      else {
        projs = projs.filter(x => x !== 'ทั้งหมด');
        if (!projs.includes(pj)) {
           if (projs.length >= 15) return alert('1 อีเมลสามารถผูกโครงการได้สูงสุด 15 โครงการครับ');
           projs.push(pj);
        }
      }
      nEms[idx] = `${em}|${projs.join(',')}`;
    } else { nEms.push(`${em}|${pj}`); }
    
    saveD('settings', {...sets, emails: nEms}); setSInp({...sInp, emails: '', emProj: 'ทั้งหมด'});
  };

  const rmEmailProj = (emStr, pRm) => {
    const parts = emStr.split('|'), em = parts[0];
    let projs = parts[1].split(',').filter(x => x !== pRm);
    let nEms = sets.emails.filter(x => x !== emStr);
    if (projs.length > 0) nEms.push(`${em}|${projs.join(',')}`);
                ...
                <tbody>
                  {ft.map(j => (
                    <tr key={j.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3">
                        <div className="font-bold text-[#0f2e4a] text-sm">{j.project} <span className="font-normal text-xs text-gray-500">({j.area})</span></div>
              ...
              <div className="flex-1 overflow-y-auto space-y-3 hide-scrollbar">
                {ubGrp.map(g => (
                  <div key={g.id} draggable onDragStart={e=>oDS(e, g.id)} className="bg-white p-3 rounded-lg shadow-sm border border-gray-200 cursor-move hover:border-blue-400 relative">
                    ...
                    <div className="font-bold text-[#0f2e4a] text-sm mb-1">{g.project} <span className="text-xs text-gray-500 font-normal">({g.tasks.length} งาน)</span></div>
              ...
              <div className="flex-1 overflow-y-auto space-y-3 hide-scrollbar">
                {biGrp.map(g => (
                  <div key={g.id} draggable onDragStart={e=>oDS(e, g.id)} className="bg-white p-3 rounded-lg shadow-sm border border-green-200 cursor-move hover:border-green-400 relative">
                    ...
                    <div className="font-bold text-green-800 text-sm mb-1">{g.project} <span className="text-xs text-green-600/70 font-normal">({g.tasks.length} งาน)</span></div>
  ...
  const PReport = () => {
    ...
    // 🛠️ 2. กรองข้อมูลตามช่วงเวลาและพื้นที่
    ...
    return (
      <div id="print-area" className="hidden p-8 font-sans bg-white">
        <div className="text-center border-b-2 border-[#0f2e4a] pb-4 mb-6">
          <h1 className="text-2xl font-bold text-[#0f2e4a] uppercase">รายงานผลการดำเนินงาน LH Task-Flow</h1>
          <p className="text-sm text-gray-600 mt-2 font-bold">รอบ: {isY ? `ปี ${fS}` : `เดือน ${fS}`} | พื้นที่: {rCfg.area} | โครงการ: {rCfg.project}</p>
        </div>
        <div className="flex gap-4 mb-8 print-break">
          <div className="flex-1 bg-gray-50 border p-4 rounded-lg text-center"><p className="text-xs text-gray-500 font-bold">ปริมาณงานที่ได้รับ</p><h2 className="text-2xl font-black">{rT.length}</h2></div>
          <div className="flex-1 bg-green-50 border p-4 rounded-lg text-center"><p className="text-xs text-green-700 font-bold">งานที่จบแล้ว</p><h2 className="text-2xl font-black text-green-700">{rC.length}</h2></div>
          <div className="flex-1 bg-yellow-50 border p-4 rounded-lg text-center"><p className="text-xs text-yellow-700 font-bold">ดำเนินการ</p><h2 className="text-2xl font-black text-yellow-700">{rO.length}</h2></div>
          <div className="flex-1 bg-red-50 border p-4 rounded-lg text-center"><p className="text-xs text-red-700 font-bold">เกินกำหนด</p><h2 className="text-2xl font-black text-red-700">{rOd.length}</h2></div>
        </div>
        
        <div className="mb-8 p-4 border rounded-lg bg-gray-50 print-break">
          <h3 className="font-bold text-[#0f2e4a] mb-2 text-sm border-b pb-2">สรุปส่งเบิก (เฉพาะงานที่จบแล้ว)</h3>
          <div className="flex justify-between px-4 text-sm mb-2">
            <div><span className="font-bold text-green-600">ส่งเบิกแล้วทั้งหมดในระบบ:</span> {b} รายการ</div>
            <div><span className="font-bold text-red-600">ค้างเบิก (สะสมทั้งหมด):</span> {ub} รายการ</div>
          </div>
          {ub > 0 && (
            <div className="px-4 text-[11px] mt-3 border-t pt-3 text-gray-600 flex flex-wrap gap-2 items-center">
              ...
              {sortedUbMonths.map(m => (<span key={m} className="bg-white border border-gray-300 px-2 py-0.5 rounded shadow-sm text-red-600 font-bold">{m} : {ubBreakdown[m]} รายการ</span>))}
            </div>
          )}
        </div>
        
        <div className="mb-8 print-break">
          <h3 className="font-bold text-[#0f2e4a] mb-4 text-sm border-b pb-2">สถานะงานแยกตามโครงการ</h3>
          <div className="space-y-3">
            {Object.keys(pSt).map(p => { 
              const s = pSt[p]; 
              return (
                <div key={p} className="flex items-center text-xs">
                  <div className="w-1/4 font-bold truncate pr-2" title={p}>{p}</div>
                  <div className="w-2/4 bg-gray-200 h-5 rounded overflow-hidden flex">
                    {s.t>0 && <div style={{width:`${(s.d/s.t)*100}%`}} className="bg-green-500 h-full"></div>}
                    {s.t>0 && <div style={{width:`${(s.o/s.t)*100}%`}} className="bg-yellow-400 h-full"></div>}
                    {s.t>0 && <div style={{width:`${(s.od/s.t)*100}%`}} className="bg-red-500 h-full"></div>}
                  </div>
                  <div className="w-1/4 pl-3 text-[10px] text-gray-500">รวม {s.t} (จบ:{s.d}, ทำ:{s.o}, ช้า:{s.od})</div>
                </div>
              ); 
            })}
          </div>
          <div className="flex gap-4 text-[10px] justify-center mt-4 font-bold">
            <div className="flex items-center"><span className="w-3 h-3 bg-green-500 rounded-sm mr-1"></span>จบงาน</div>
            <div className="flex items-center"><span className="w-3 h-3 bg-yellow-400 rounded-sm mr-1"></span>กำลังดำเนินการ</div>
            <div className="flex items-center"><span className="w-3 h-3 bg-red-500 rounded-sm mr-1"></span>เกินกำหนด</div>
          </div>
        </div>

        <div className="print-break">
          <h3 className="font-bold text-[#0f2e4a] mb-2 text-sm border-b pb-2">งานที่ถูกยกเลิก</h3>
          <table className="w-full text-[11px] text-left border-collapse border">
            ...
            <tbody>
              {rCancel.map(t=>(
                <tr key={t.id}>
                  <td className="border p-2 font-bold text-blue-700">{t.workOrderNo || t.id}<br/><span className="text-gray-600 font-normal">{getStdName(t.project)}</span></td>
                  <td className="border p-2">{t.details}</td>
                  <td className="border p-2 text-gray-500">ยกเลิก</td>
                  <td className="border p-2 text-red-600 font-bold">{t.cancelReason}</td>
                </tr>
              ))}
              {rCancel.length === 0 && <tr><td colSpan="4" className="border p-4 text-center text-gray-400">ไม่มีงานที่ถูกยกเลิกในรอบนี้</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
            ...
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-[9999]">
              <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
                ...
                <form onSubmit={subT} className="p-5 space-y-3 max-h-[70vh] overflow-auto">
                  ...
                  <div className="flex gap-2">
                    <select name="requester" required defaultValue={eTask?.requester} className="w-1/2 border rounded p-2 text-sm"><option value="">ผู้แจ้ง...</option>{REQ_TYPES.map(r=><option key={r}>{r}</option>)}</select>
                    <select name="slaCategory" defaultValue={eTask?.slaCategory} className="w-1/2 border rounded p-2 text-sm"><option value="">หมวด SLA (ถ้ามี)...</option>{sets.slas.map(s=><option key={s} value={getProjName(s)}>{getProjName(s)} ({getProjArea(s)} วัน)</option>)}</select>
                  </div>

                  <div className="flex gap-2">
                    <select name="project" required defaultValue={eTask?.project} onChange={(e)=>{ const p = sets.projects.find(x=>getProjName(x)===e.target.value); if(p) document.getElementById('task_area').value = getProjArea(p); }} className="w-2/3 border rounded p-2 text-sm"><option value="">โครงการ (เลือกเพื่อดึงพื้นที่)...</option>{sets.projects.map(p=><option key={p} value={getProjName(p)}>{getProjName(p)}</option>)}</select>
                    <input type="text" id="task_area" name="area" required readOnly defaultValue={eTask?.area} placeholder="พื้นที่..." className="w-1/3 border rounded p-2 text-sm bg-gray-100" />
                  </div>

                  <div className="flex gap-2">
                    <div className="w-1/2"><label className="text-[10px] font-bold">เริ่มงาน</label><input type="date" name="startDate" required defaultValue={eTask?.startDate||getTStr()} onChange={(e)=>eTask && setShowStartReason(e.target.value !== eTask.startDate)} className="w-full border rounded p-2 text-sm" /></div>
                    <div className="w-1/2"><label className="text-[10px] font-bold">กำหนดเสร็จ</label><input type="date" name="endDate" required defaultValue={eTask?.endDate||getTStr()} className="w-full border rounded p-2 text-sm" /></div>
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  {sMod.type==='complete' && (
                    <div>
                      ...
                      <input type="text" placeholder="ตัวอย่าง: LH-123-1234567" className="w-full border border-green-300 bg-green-50 rounded p-2 text-sm uppercase" value={sMod.workOrderNo} onChange={e=>setSMod({...sMod,workOrderNo:e.target.value.toUpperCase()})} />
                    </div>
                  )}
                </div>
                <div className="flex gap-2 mt-5">
                  <button type="button" onClick={()=>setSMod({...sMod,isOpen:false})} className="flex-1 bg-gray-100 p-2 text-xs font-bold rounded">ปิด</button>
                  <button type="button" onClick={cfSt} disabled={(sMod.type==='cancel'&&!sMod.reason.trim())||(sMod.type==='complete'&&!sMod.workOrderNo.trim())} className="flex-1 bg-[#0f2e4a] text-white p-2 text-xs font-bold rounded disabled:opacity-50">ยืนยัน</button>
                </div>
              </div>
            </div>
          )}
