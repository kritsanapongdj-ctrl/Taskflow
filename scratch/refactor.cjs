...
const cinematic_view_code = `
    const renderCinematicView = () => {
      ...
      return (
        <div className="flex-1 w-full bg-[#08080c] relative overflow-hidden flex flex-col md:flex-row shadow-[inset_0_0_100px_rgba(0,0,0,0.8)] rounded-xl">
...
const target_block = \`          {(!selTeam && !teamForm.name && !teamForm.id) ? (
...
const replacement_block = \`          {(!selTeam && !teamForm.name && !teamForm.id) ? (
             <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-10">
               <Icon name="users" size={64} className="mb-4 opacity-50" />
               <p className="font-bold">เลือกทีมงานจากเมนูด้านซ้าย</p>
               <p className="text-xs mt-1">หรือกด "+ เพิ่ม" เพื่อสร้างโปรไฟล์ใหม่</p>
...
const cancel_btn_target = \`<button type="button" onClick={saveTeam} className="flex-1 bg-[#0f2e4a] text-white py-2.5 rounded-lg font-bold shadow-md hover:bg-[#1a3f63] flex justify-center items-center"><Icon name="save" size={16} className="mr-2"/> บันทึกข้อมูล</button>\`;
const cancel_btn_replace = \`<button type="button" onClick={saveTeam} className="flex-1 bg-[#0f2e4a] text-white py-2.5 rounded-lg font-bold shadow-md hover:bg-[#1a3f63] flex justify-center items-center"><Icon name="save" size={16} className="mr-2"/> บันทึกข้อมูล</button>
                        {teamForm.id && <button type="button" onClick={()=>{setTeamEditMode(false); setSelTeam({...teamForm})}} className="bg-slate-100 text-slate-600 px-4 rounded-lg font-bold hover:bg-slate-200">ยกเลิก</button>}\`;
code = code.replace(cancel_btn_target, cancel_btn_replace);

const end_target = \`                </div>
             </div>
          )}
        </div>
      </div>
